...
def gen_dot(answer: str, is_attention: bool, C) -> dict:
    if is_attention:
...
def gen_gabor(answer: str, difficulty: float = 15.0) -> dict:
...
def gen_rdk(answer: str, coherence: float = 5.0) -> dict:
    return dict(
        direction = answer,
        coherence = coherence,
        n_dots = 200,
...
def gen_avg(answer: str, mean_shift: float = 0.15, sigma: float = 0.45, n_elements: int = 8) -> dict:
...
def generate_stimulus(task: dict, C) -> dict:
    ...
    if t == 'dot':
        return gen_dot(ans, is_attn, C)
    elif t == 'gabor':
        return gen_gabor(ans, task.get('difficulty', 3.5))
    elif t == 'rdk':
        return gen_rdk(ans, task.get('coherence', 10.0))
    elif t == 'avg':
        return gen_avg(ans, task.get('mean_shift', 0.15))
    else:
